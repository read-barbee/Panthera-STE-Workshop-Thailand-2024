---
title: "Module 04: Estimating abundance with IS"
...
In this module, we will discuss the inputs and data structures required to estimate abundance of a target species using Instantaneous Sampling (IS) in the spaceNtime package in R. By the end of this module, you should understand what data structures are required for IS, how to prepare the input files, and how to get an initial abundance estimate for your target species.

## Background: What kind of data do we need for IS?

As with Space to Event (STE) models, Instantaenous Sampling (IS) requires TIIMELAPSE data. This means that all cameras are programmed to take photos at regular intervals regardless of whether or not an animal is present. 

We can run an IS analysis using the same inputs that we used for the STE model. The primary difference is that for IS, the count column must be actual **counts** of the target animals in each photo. In contrast, STE can be run with counts or presence/absence data. 

we will need two separate data files:
...
- **count**: The count of your study species at each date and time. For IS, this must be a **count**, not simply species presence or absence (0 or 1) (class numeric)
...
Like STE models, IS models are designed to be run on time-lapse photo data, but they can also be used with motion-trigger data. We will start with the motion-triggered version since our data is already in that format. Then we will repeat the analyis with a new "timelapse" version of the dataset.
...
- **boar**: the number of individuals of the target species detected in each photo. In this case, our values range from 0 to 3. For IS, we need to know the number of animals in each photo, not just presence/absence. (class: double)
...
Now we're ready to build our IS encounter history! This is the final data object that we will use to run our model. 
...
`r knitr::kable(head(ise_eh), caption = "IS encounter history")`

The encounter history looks slightly different than the one for STE. It has one row for every sampling occasion and contains the following columns:

-**occ**: the number of the sampling occasion from our "deploy" object. (class: integer)

-**cam**: the name of the camera station. (class: any)
...
-**area**: the viewable area of the camera at each sampling occasion. (class: numeric)

-**count**: The count of target species at each camera on each sampling occasion. If a camera was inactive or not functioning, the value is NA.

There is no censor column anymore because we are no longer estimating time or space to an event.


## Step 7: Estimate abundance!

Now that we've done the hard work of wrangling our data into an encounter history, we can estimate the IS abundance using one simple line of code! All we need is the ise_estN_fn from the spaceNtime package. We only need to give the function 2 components:
...
```{r}
ise_estN_fn(ise_eh, study_area = 1 * 1e6)
```

So we estimate that there are about 2.6 boar per km2 in our study area with a 95% confidence interval of 1 to 6.4.
...
tl_data <- read_csv("boar_timelapse_example_data_SLP_v4.csv", show_col_types = FALSE) %>% 
  janitor::clean_names()
...
```{r}
ise_eh_tl <- ise_build_eh(df_tl, deploy_tl, occ_tl)
```

`r knitr::kable(head(ise_eh_tl), caption = "IS encounter history (Timelapse)")`


## Step 6: Estimate abundance!

Our final step is also the same as before. All we need is the ise_estN_fn from the spaceNtime package. Again, be sure to specify your study_area size in the same units as your camera visible areas! 

```{r}
ise_estN_fn(ise_eh_tl, study_area = 1 * 1e6)
```

This estimate is far too high, so there may be an issue with the underlying timelapse data. You can still use the code for your own dataset though.
