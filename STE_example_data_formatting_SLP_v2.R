...
cam_data <- read_csv("SCL/SLP/SLP_2022_CameraDetails.csv") %>% 
  janitor::clean_names() %>% 
...
for(i in 1:nrow(cam_data)){
  ...
  station_fill <- cam_data$station[i]
  start_dt <- ymd_hms(paste(as.character(cam_data$start_date[i]), "00:00:00"))
  end_dt <- ymd_hms(paste(as.character(cam_data$end_date[i]), "00:00:00")) + days(1)
  
  ts_complete[[i]] <- tibble(
    station = station_fill,
    date_time = seq(from = start_dt, to = end_dt, by = "hour")
  )
}

ts_complete <- bind_rows(ts_complete)

###############################################################################

#2. Get boar detections
...
tolerance <- 0 # tolerance for matching in seconds

result <- difference_left_join(
  ts_complete, boar, 
  by = c("date_time" = "date_time_original"), 
  max_dist = tolerance, 
  distance_col = "time_difference"
) %>% distinct(station.x, date_time, .keep_all = T)
...
result <- result %>% 
  ...
  mutate(species = replace_na(species, species_fill),
         area_m2 = area_fill,
         group = replace_na(group, group_size_fill)) %>% 
  select(station, species, date_time, group, area_m2)


split2 <- result %>% split(result$group)


# split2$`0` <- split2$`0` %>%
#   mutate(tf = rbinom(nrow(split2$`0`), 1, 0.7)) %>%
#   mutate(area_m2 = case_when(tf == 1 ~ area_m2,
#                              tf == 0 ~ 0)) %>%
#   select(-tf)

split2$`1` <- split2$`1` %>% 
  mutate(tf = rbinom(nrow(split2$`1`), 1, 0.99)) %>% 
  mutate(group = case_when(tf == 1 ~ group,
                           tf == 0 ~ 2)) %>% 
  select(-tf)

split2$`1` <- split2$`1` %>% 
  mutate(tf = rbinom(nrow(split2$`1`), 1, 0.999)) %>% 
  mutate(group = case_when(tf == 1 ~ group,
                           tf == 0 ~ 3)) %>% 
  select(-tf)
...
final <- tl_data %>% distinct(station, date_time, .keep_all = T)

#write_csv(final, "boar_timelapse_example_data_SLP_v5.csv")
...
#2. Match boar detections to nearest sampling occasion

###############################################################################

matched_records <- boar_detections %>%
  rowwise() %>%
  filter(any(abs(difftime(timestamp, timelapse_timestamps$timestamp, units = "secs")) <= 10))
